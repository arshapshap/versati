...
import com.arshapshap.versati.feature.imageparsing.data.network.response.ParsedImageRemote
...
internal class ImageParsingMapper {
    ...
    fun mapFromLocal(local: ParsingResultLocal): ParsingResult {
        return ParsingResult(
            id = local.id,
            parsedResults = local.parsedText.split(PARSED_TEXT_SEPARATOR).map { ParsedImage(parsedText = it) },
